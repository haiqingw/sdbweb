  ...
  <div>
    ...
    <div class="certificationMain">
      ...
      <div class="itemTopTextMain">
        ...
        <div class="line_bottom">
          <input
            type="text"
            placeholder="请输入手机号"
            v-model="queryData.forgetParam.phone"
          />
        </div>
      </div>
      <div class="itemTopTextMain">
      ...
      <div class="submitBtnMain">
        <mt-button type="primary" size="large" @click="goToNext"
          >下一步</mt-button
        >
      </div>
    </div>
  ...
  methods: {
    goToNext() {
      if (
        ...
        !/^1(3|4|5|7|8)\d{9}$/.test(this.queryData.forgetParam.phone)
      ) {
        Toast({
          message: "手机号格式不正确",
          duration: 1000
      ...
      // 执行结果
      if (this.flag) {
        if (this.queryData.forgetParam.verificationCode == this.Verify) {
          this.$router.push({ name: "forgetPasswordNext" ,params:{
